...
libs = c("ggplot2", "parallel", "reshape2", "nlme", "glmnet", "grid", "mvtnorm", 
         "xtable", "pengls", "numDeriv", "BiocParallel", "ks", "ashr", "nlpden",
...
colourVec = c("rawEst" = "black", "split" = "#000099", "estOutSplit" = "grey25", "condML" = "#990099", 
              "FayeCorrectEst" = "#FF6600", "TanCorrectEst" = "#FFCC00", "TanCorrectEstParam" = "#CCCC99", "bootCorrectForde" = "#FF0099", "tweedieEst" = "#CCCC99", "tweedieEstsTrunc" = "#CCCC99",
...
methodLabels = c("Raw", "Separate estimation", "Separate estimation", "Conditional likelihood", 
                 "BR-squared", "Tan2015 (nonparametric)", "Tan2015 (parametric)", "winnerscurse", "Tweedie's formula", "Tweedie's formula\ntruncated", "Tweedie's formula\nconvoluted or bagged", "Tweedie's formula\nconvoluted or bagged\n(truncated)", 
...
foo = lapply(c("Results", "simResults"), dir.create, showWarnings = FALSE) #Create paths to store results
options(tinytex.clean = FALSE) #Keep aux file for cross-referencing
```

\tableofcontents

The R-code for producing all output of this study is available from _Biostatistics_ online.
...
# Convolution for improved density estimation under dependence
...
Although convolution yields an estimator $\tilde{f}$ with correct expected variance, unbiasedness of the estimation of the derivative of its log-density and hence of Tweedie's formula cannot be guaranteed in general. In computer simulations we see a great improvement in the bias though, suggesting that convolution is a simple and good working solution that drastically reduces bias in Tweedie's formula under many scenarios. Here we prove unbiasedness of our procedure in a few simple cases. We assume that M=1, so the covariance structure is determined by a single confounder that engenders a compound symmetric covariance structure, as is the case in our motivating example of estimating prediction loss for a single outcome using many possible predictors. In that case $\hat{k}$ is a normal distribution $\hat{k}(z) = r(z|\zeta \sqrt{\alpha_1}, 1-\alpha_1)$, with $\zeta$ standard normal. Since we work with z-values, the true $k$ is an even mixture of standard normals and thus itself standard normal: $k(z) = \phi(z)$.
...
In the main text, the theoretical results from Sections \ref{sec:tweedieBiased}-\ref{sec:solution} are validated using Monte-Carlo simulations in Section \ref{sec:Tweedieconv}, as described in Section \ref{sec:simparam}. Here we provide exhaustive results, including not only the compound symmetric correlation structure but also block correlation.
...
ggplot(data = moltMeans[moltMeans$what=="Bias" & abs(moltMeans$z) < 1,], aes(x = z, y = value, col = factor(p))) +
    ...
    ylab("Bias")
ggsave("ManuscriptSelBias/Graphs/LogDerivMean.pdf", height = 6.5, width = 8) 
...
ggplot(data = molt2[molt2$what=="bias" & abs(molt2$z) < 4,], aes(x = z, y = value, col = factor(estimator))) +
...
ggsave("ManuscriptSelBias/Graphs/LogDerivMean2.pdf", height = 6.5, width = 8)
```
...
plotNormal = c("rawEst", "FayeCorrectEst", "TanCorrectEst","TanCorrectEstParam",
               "tweedieConvEsts", "split", "condML", "ash",
               "ashConv", "catScores", "bootCorrectForde", "VanZwet2021" , "VanZwet2021conv")
plotNormalPaper = c("rawEst", "FayeCorrectEst", "TanCorrectEst", "VanZwet2021conv",
               "tweedieConvEsts", "split", "condML", "ashConv", "catScores", "bootCorrectForde")
```
...
plotNormalResc = plotNormalResc[!(plotNormalResc %in% c("catScores_rescale", "VanZwet2021_rescale", "VanZwet2021conv_rescale"))]
...
if(!file.exists(fullFileSup <- "Results/fullDfSup.RData")){
    baseTestVarCIresc = lapply(baseTestVarCI, function(xx){
        lapply(xx, function(x){
            lapply(x, function(y){
                #Raw estimates
                tmp = y$resMatZ[, setdiff(plotNormal, c("rawEst", "condML", "split", "VanZwet2021conv"))] * (ses <- sqrt(y$resMat[, "Vars"]))
...
fullDfDiff2 = rbind(fullDfDiff, fullDfDiffTdr11, fullDfDiffTdr2, fullDfDiffTdr3)
fullDfDiff2$what = factor(fullDfDiff2$what, levels = c("Test statistic - raw", "Rescaled test statistic - raw"), ordered = TRUE)
...
The rank conditional coverage (RCC) is shown in \fref{fig:rcc} for the parametric simulation study (dense scenario), reflecting the average coverage at each rank separately. Yet there is no material difference between the coverage results of the RCC and overall coverage, except that the RCC is more variable.
...
if(!file.exists(UniMSEcvFileSplit <- "Results/UniMSECVsplit.RData")){ 
    ...
    estsListSplit = lapply(names(phenId1), function(phen){
        ...
        phenRes = mclapply(mc.cores = 2, UniMSECVsplit[[phen]], function(y){
                ...
                bootCorrectForde = forde(rawEsts, Vars <- seEsts^2, allowInflate = TRUE)
...
if(!file.exists(estsListCIfile <- "Results/estsListsCI.RData")){
    ...
    estsListCI = mclapply(names(phenId1), mc.cores = 1, function(phen){
        ...
        ashObj = ash(rawEsts, seEsts, method = "shrink", mode = "estimate")
        bootCorrectForde = forde(rawEsts, seEsts^2, allowInflate = TRUE)
...
ciSmallest = vapply(FUN.VALUE = double(2), seq_len(nrow(smallestEstWhich)), function(i){
    ...
    if(x[["Method"]] %in% c("Tan2015 (nonparametric)", "Cat scores\nnonparametric bootstrap", "BR-squared", "winnerscurse")){return(c(NA, NA))}
...
ggsave("ManuscriptSelBias/Graphs/BrasPredReal.pdf", height = 8, width = 7.5) 
```
...
if(!file.exists(benchCompFile <- "simResults/benchComp.RData")){
    benchComp = vapply(seq_len(benchReps), FUN.VALUE = matrix(0, 14, 2), function(j){
                    ...
                    ashObj = getConvAsh(ests, ses, alpha1 = alpha1, ashObj = ash(betahat = ests, ses, method = "shrink", mode = "estimate"))
                        ashci(ashObj, level = 1-sigLevel, trace = FALSE)
                    })[3], "catScores" = NA, "bootCorrectForde" = NA,
                                      "VanZwet2021" = system.time({
                       ests = colMeans(mat)
                       s = apply(mat, 2, sd)/sqrt(nrow(mat))
                       mr = mean(ests)
...
The software versions used are detailed below.
