...
### Convolution estimator \label{sec:convEst}
...
## Empirical validation of density bagging on all estimates combined \label{sec:bagging}

```{r defBag}
zExt = c(-1, 1)*1.6; zSeqBag = seq(zExt[1], zExt[2], length.out = 1e3)
repsBag = 9;pBag = 5e3;sdBag = 0
```

Here we validate empirically that estimating the density on the collection of all bootstrapped estimates is equivalent to the classical bagging approach, whereby separate densities are estimated per bootstrap sample and then these densities are averaged. Nine data matrices are generated as in Section \ref{sec:convEst} with p=`r pBag` and zero correlation, `r B` bootstrap instances are taken, and the densities are estimated through Lindsey's estimator \parencite{Lindsey1974} in the two ways described above. \fref{fig:bagging} demonstrates how similar the densities are, justifying the estimation on all bootstrap estimates jointly while still calling it "density bagging".

```{r bagging, fig.height = 6.5, fig.cap = "\\label{fig:bagging}Nine instances of density estimates obtained from the observed estimates (red dotted line), by averaging densities estimates on separate bootstrap samples (light blue solid line) and estimated on the collection of all bootstrap samples jointly (black dashed line). The latter two density estimates are so similar that the lines overlap almost completely. This graph also illustrates how bagging widens the density estimates."}
Vars = runif(pBag, varRange[1], varRange[2]);sds = sqrt(Vars)
mat = rmvnorm(n = repsBag*n, mean = rep(0, pBag), sigma = makeSigma(sdBag, pBag)*outer(sds, sds))
tmp = double(length(zSeqBag))
outBag = vapply(idBag <- seq_len(repsBag), FUN.VALUE = matrix(0, length(zSeqBag), 3), function(j){
        matIn = mat[seq_len(n) + (j-1)*n,]
        obsMeans = colMeans(matIn)
        obsDensAll = exp(evalFitLog(estDens(z = obsMeans, densMethod = "Lindsey", returnFit = TRUE), z = zSeqBag))
        obsDensAll = obsDensAll/sum(obsDensAll)
        bootMeans = vapply(integer(B), FUN.VALUE = double(pBag), function(i){
            colMeans(matIn[sample(n, replace = TRUE),])
        })
        bootDensAll = exp(evalFitLog(estDens(z = c(bootMeans), densMethod = "Lindsey", returnFit = TRUE), z = zSeqBag))
        bootDensAll = bootDensAll/sum(bootDensAll)
        bootDensSep = apply(bootMeans, 2, function(x){
            id = zSeqBag > min(x) & zSeqBag < max(x)
             tmp[id] = exp(evalFitLog(z = zSeqBag[id], estDens(z = x, densMethod = "Lindsey", returnFit = TRUE)))
             #Avoid extrapolation
             tmp/sum(tmp)
        })
        bootDensSepAv = rowMeans(bootDensSep)
        cbind('ObsEstimation' = obsDensAll, 'JointEstimation' = bootDensAll, 'SeparateEstimation' = bootDensSepAv)
})
par(mfrow = c(3,3), mar = c(4,4,4,4))
foo = lapply(idBag, function(xx){
    plot(zSeqBag, outBag[, 'ObsEstimation', xx], col = "red", lty = "dotted", type = "l", xlab = "z-values", ylab = "Density")
    lines(zSeqBag, outBag[, 'JointEstimation', xx], col = "lightblue")
    lines(zSeqBag, outBag[, 'SeparateEstimation', xx], col = "black", lty = "dashed")
})
par(mfrow = c(1,1))
```
